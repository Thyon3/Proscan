...
class AppStorageService {
  ...
  Future<String> getAppDirectory() async {
    try {
      ...
      if (!await appDir.exists()) {
        ...
        AppLogger.info('Created ThyScan main directory', data: {'path': appDir.path});
      }
  ...
  Future<String> getScanModeFolderPath(ScanMode scanMode) async {
    try {
      ...
      if (!await folderPath.exists()) {
        ...
        AppLogger.info('Created scan mode folder', 
            data: {'mode': scanMode.name, 'path': folderPath.path});
      }
      
      return folderPath.path;
    } catch (e, stack) {
      AppLogger.error('Failed to get scan mode folder path', 
          error: e, stack: stack, data: {'scanMode': scanMode.name});
      rethrow;
    }
  }
  ...
  Future<String> getScanModeFolderPathFromString(String scanMode) async {
    try {
      ...
      if (!await folderPath.exists()) {
        await folderPath.create(recursive: true);
        AppLogger.info('Created scan mode folder from string', 
            data: {'scanMode': scanMode, 'path': folderPath.path});
      }
      
      return folderPath.path;
    } catch (e, stack) {
      AppLogger.error('Failed to get scan mode folder path from string', 
          error: e, stack: stack, data: {'scanMode': scanMode});
      rethrow;
    }
  }
  ...
  Future<String> moveToAppFolder({
    ...
    try {
      ...
      if (await finalFile.exists()) {
        AppLogger.warning('Target file already exists, deleting old version',
            data: {'path': finalPath});
      ...
      AppLogger.info('Moved document to organized folder',
          data: {
      ...
      AppLogger.error('Failed to move document to app folder',
          error: e, stack: stack,
          data: {
  ...
  Future<String> getFinalDocumentPath({
    ...
    try {
      ...
      AppLogger.error('Failed to get final document path',
          error: e, stack: stack,
          data: {
            'documentId': documentId,
            'scanMode': scanMode,
  ...
  Future<void> openScanModeFolder(ScanMode scanMode) async {
    try {
      ...
      AppLogger.info('Open scan mode folder requested',
          data: {'mode': scanMode.name, 'path': folderPath});
      
      // TODO: Implement actual file manager opening
    } catch (e, stack) {
      AppLogger.error('Failed to open scan mode folder',
          error: e, stack: stack, data: {'scanMode': scanMode.name});
    }
  }
}
